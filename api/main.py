...
from monai_pipeline.findings_classifier import MockFindingsClassifier
...
findings_classifier: Optional[MockFindingsClassifier] = None
...
async def startup_event():
    ...
    # Findings classifier (mock for now)
    findings_classifier = MockFindingsClassifier()
...
async def health_check():
    ...
    return {
        ...
        "components": {
            ...
            "findings_classifier": {"ready": findings_classifier is not None, "type": "mock"},
...
async def get_available_cases():
    ...
    lidc_dir = Path("data/processed/lidc/images")
    cases = []
...
async def get_lidc_cases():
    ...
    manifest_path = Path("data/processed/lidc/manifest.json")
    cases = []
...
def get_production_pipeline():
    ...
    if production_pipeline is None:
        ...
        nodule_model_path = Path("models/nodule_det/best_nodule_det_model.pth")
        lung_seg_model_path = Path("models/lung_seg/best_lung_seg_model.pth")
        
        if nodule_model_path.exists():
            ...
            logger.info(f"Production pipeline loaded (nodule: trained, lung_seg: {'trained' if lung_seg_model_path.exists() else 'not found'})")
        else:
            logger.warning("Nodule model not found, using mock pipeline")
...
def analyze_real_lidc_case(
    ...
    # LIDC 이미지 경로
    lidc_dir = Path("data/processed/lidc")
    ...
    # Findings (모든 필수 필드 포함)
    findings = StructuredFindings(
        pleural_effusion=FindingLabel(label="absent", probability=0.05),
        pneumothorax=FindingLabel(label="absent", probability=0.02),
        consolidation=FindingLabel(label="absent", probability=0.05),
        atelectasis=FindingLabel(label="absent", probability=0.03),
        emphysema=FindingLabel(label="absent", probability=0.04)
    )
    ...
    try:
        ...
        if request.include_report:
            ...
            if draft_report and draft_report.rendered_report:
                ...
                try:
                    # Context를 풍부하게 제공 (결절 + 다른 소견)
                    context_lines = [
                        f"Study: {ai_result.study_uid}",
                        f"Nodules detected: {len(ai_result.nodules)}",
                    ]
                    for n in ai_result.nodules[:5]:
                        context_lines.append(
                            f"- {n.id}: {n.diameter_mm:.1f}mm, location={n.location_code or 'EXTRA'}, conf={n.confidence:.2f}"
                        )
                    # Absent findings 정보도 포함 (시스템 자동 생성)
                    context_lines.append("\nOther AI findings (system-generated):")
                    context_lines.append("- Pleural effusion: Absent (low confidence)")
                    context_lines.append("- Pneumothorax: Absent (low confidence)")
                    context_lines.append("- Consolidation: Absent (low confidence)")
                    context_lines.append("- Atelectasis: Absent (low confidence)")
                    context_lines.append("- Emphysema: Absent (low confidence)")
                    
                    context = "\n".join(context_lines)
                    
                    # claim은 결절 관련 부분만 추출 (FINDINGS TABLE 부분)
                    claim_text = draft_report.rendered_report[:600]
                    ...
                    logger.info(f"[{request_id}] Groundedness: {groundedness_result.is_grounded}")
                except Exception as e:
